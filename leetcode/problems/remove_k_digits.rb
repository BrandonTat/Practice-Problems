...
# Input: num = "10200", k = 1
# Output: "200"
# Explanation: Remove the leading 1 and the number is 200. Note
...
def remove_kdigits(num, k)
  ...
  num.each_char do |c|
    ...
    while result.length > 0 && k != 0 && result.last > val
      result.pop
      k -= 1
    end
  ...
  until result.empty? || k.zero?
    result.pop
    k -= 1
  end

  while result.length >= 1 && result.first.zero?
